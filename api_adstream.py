...
def new_media_creation(adstream_upload_list):
    ...
    for media in adstream_upload_list: 
        time.sleep(30)  # Have to pause between uploads or else adstream gives 10054 error
    ...
    with open("job_id_list.txt", "wt+") as f:
        for line in contents: 
            if line.strip("\n") != vantage_job_id:
                ...
                new_line = line.replace(vantage_job_id, f"[ {timestamp} - Upload Failed for job id: {vantage_job_id} ]")
                f.write(new_line)
